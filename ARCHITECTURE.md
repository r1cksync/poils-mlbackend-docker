# System Architecture - Hindi OCR System

## 🏗️ Overall Architecture

```
┌─────────────────────────────────────────────────────────────────┐
│                        USER / CLIENT                             │
│                  (Next.js Frontend - Port 3000)                  │
└───────────────────────────┬─────────────────────────────────────┘
                            │
                            │ HTTP/HTTPS
                            │
┌───────────────────────────▼─────────────────────────────────────┐
│                   Next.js Backend (Port 3001)                    │
│  ┌──────────────────────────────────────────────────────────┐  │
│  │  API Routes:                                              │  │
│  │  • /api/auth/*          - Authentication (JWT)           │  │
│  │  • /api/chats/*         - Chat management                │  │
│  │  • /api/documents/*     - Document handling              │  │
│  │  • /api/documents/ocr   - OCR processing trigger         │  │
│  └──────────────────────────────────────────────────────────┘  │
│                                                                   │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────────────┐ │
│  │  MongoDB     │  │  AWS S3      │  │  Python Backend API  │ │
│  │  (Data)      │  │  (Files)     │  │  (OCR Processing)    │ │
│  └──────────────┘  └──────────────┘  └──────────────────────┘ │
└───────────────────────────────────────────────────────────────┘
                                                │
                                                │ HTTP POST
                                                │
                        ┌───────────────────────▼──────────────────────┐
                        │   Python FastAPI Backend (Port 8000)         │
                        │   Dockerized - Deployed on Render            │
                        │                                              │
                        │  ┌────────────────────────────────────────┐ │
                        │  │  FastAPI Application                    │ │
                        │  │  • CORS middleware                      │ │
                        │  │  • Request validation (Pydantic)        │ │
                        │  │  • Error handling                       │ │
                        │  │  • Logging                              │ │
                        │  └────────────────────────────────────────┘ │
                        │                                              │
                        │  ┌────────────────────────────────────────┐ │
                        │  │  Image Processor Service                │ │
                        │  │  • Format conversion                    │ │
                        │  │  • Validation                           │ │
                        │  │  • Preprocessing (grayscale, denoise)   │ │
                        │  │  • Resizing                             │ │
                        │  └────────────────────────────────────────┘ │
                        │                                              │
                        │  ┌────────────────────────────────────────┐ │
                        │  │  Model Service                          │ │
                        │  │  • Microsoft TrOCR Model                │ │
                        │  │  • PyTorch Inference                    │ │
                        │  │  • Transformers Pipeline                │ │
                        │  │  • Text extraction                      │ │
                        │  └────────────────────────────────────────┘ │
                        │                                              │
                        │  ┌────────────────────────────────────────┐ │
                        │  │  Model Cache (Persistent Disk)          │ │
                        │  │  • Pre-trained weights (~1GB)           │ │
                        │  │  • Tokenizer configs                    │ │
                        │  └────────────────────────────────────────┘ │
                        └──────────────────────────────────────────────┘
```

## 🔄 Request Flow - Document Upload & OCR

```
1. USER UPLOADS DOCUMENT
   │
   ├─► Frontend (Next.js)
   │   └─► File validation (size, type)
   │
2. SEND TO NEXT.JS BACKEND
   │
   ├─► POST /api/documents/ocr
   │   ├─► Verify JWT token
   │   ├─► Extract user ID
   │   └─► Receive file
   │
3. STORE IN AWS S3
   │
   ├─► Generate unique S3 key
   ├─► Upload to S3 bucket
   └─► Get S3 URL
   │
4. CREATE DOCUMENT RECORD
   │
   ├─► MongoDB.Document.create()
   │   ├─► userId
   │   ├─► fileName
   │   ├─► s3Key
   │   └─► status: "processing"
   │
5. SEND TO PYTHON BACKEND
   │
   ├─► POST {PYTHON_BACKEND_URL}/api/ocr/extract
   │   └─► FormData with image file
   │
6. PYTHON BACKEND PROCESSING
   │
   ├─► Validate image
   │
   ├─► Preprocess image
   │   ├─► Convert to grayscale
   │   ├─► Apply thresholding
   │   ├─► Denoise
   │   └─► Resize if needed
   │
   ├─► Load TrOCR Model (if not loaded)
   │   ├─► Vision Encoder (DeiT)
   │   └─► Text Decoder (RoBERTa Hindi)
   │
   ├─► Run Inference
   │   ├─► Extract pixel values
   │   ├─► Generate text tokens
   │   └─► Decode to Hindi text
   │
   └─► Return results
       ├─► text (extracted Hindi)
       ├─► confidence (0-1)
       └─► processing_time (seconds)
   │
7. UPDATE DOCUMENT IN MONGODB
   │
   ├─► Document.findByIdAndUpdate()
   │   ├─► ocrText: extracted text
   │   ├─► ocrConfidence: confidence score
   │   └─► status: "completed"
   │
8. RETURN TO FRONTEND
   │
   └─► Response JSON
       ├─► success: true
       ├─► text: "निकाला गया पाठ"
       └─► documentId
```

## 📊 Data Flow Diagram

```
┌──────────────┐
│   Browser    │
│  (User)      │
└──────┬───────┘
       │
       │ 1. Upload Document
       ▼
┌──────────────────┐
│  Next.js         │
│  Frontend        │
│  (Port 3000)     │
└──────┬───────────┘
       │
       │ 2. POST /api/documents/ocr
       ▼
┌──────────────────┐      3. Store File      ┌──────────────┐
│  Next.js         ├──────────────────────────►   AWS S3     │
│  Backend         │                           │  Bucket      │
│  (Port 3001)     │                           └──────────────┘
└──────┬───────────┘
       │
       │ 4. Save metadata
       ▼
┌──────────────────┐
│    MongoDB       │
│   (Atlas)        │
│  - userId        │
│  - fileName      │
│  - s3Key         │
│  - status        │
│  - ocrText       │
└──────────────────┘
       │
       │ 5. Send for OCR
       ▼
┌────────────────────────┐
│  Python FastAPI        │
│  (Port 8000 / Render)  │
│                        │
│  ┌──────────────────┐  │
│  │ Image Processor  │  │
│  └────────┬─────────┘  │
│           │            │
│  ┌────────▼─────────┐  │
│  │ TrOCR Model      │  │
│  │ (Indic-TrOCR)    │  │
│  └────────┬─────────┘  │
│           │            │
│  ┌────────▼─────────┐  │
│  │ Text Output      │  │
│  └──────────────────┘  │
└───────────┬────────────┘
            │
            │ 6. Return OCR result
            ▼
       ┌──────────────┐
       │   Response   │
       │  {           │
       │   text: "...",│
       │   confidence  │
       │  }           │
       └──────────────┘
```

## 🔐 Authentication Flow

```
┌─────────┐
│  User   │
└────┬────┘
     │
     │ 1. POST /api/auth/signup
     ▼
┌─────────────────┐
│   Next.js API   │
│                 │
│  • Hash pwd     │
│  • Create user  │
│  • Generate JWT │
│  • Set cookie   │
└────┬────────────┘
     │
     │ 2. JWT Token
     ▼
┌─────────────────┐
│   MongoDB       │
│   User stored   │
└─────────────────┘
     │
     │ 3. Redirect to dashboard
     ▼
┌─────────────────┐
│   Protected     │
│   Routes        │
│   (Middleware)  │
└─────────────────┘
```

## 🐳 Docker Container Structure

```
┌────────────────────────────────────────────┐
│  Docker Container (Python Backend)          │
│                                             │
│  ┌────────────────────────────────────┐    │
│  │  Python 3.9 Slim                   │    │
│  └────────────────────────────────────┘    │
│                                             │
│  ┌────────────────────────────────────┐    │
│  │  System Dependencies               │    │
│  │  • libgl1-mesa-glx (OpenCV)       │    │
│  │  • poppler-utils (PDF)             │    │
│  │  • libglib2.0-0                    │    │
│  └────────────────────────────────────┘    │
│                                             │
│  ┌────────────────────────────────────┐    │
│  │  Python Packages                   │    │
│  │  • fastapi                         │    │
│  │  • uvicorn                         │    │
│  │  • torch                           │    │
│  │  • transformers                    │    │
│  │  • Pillow, OpenCV                  │    │
│  └────────────────────────────────────┘    │
│                                             │
│  ┌────────────────────────────────────┐    │
│  │  Application Code                  │    │
│  │  /app/main.py                      │    │
│  │  /app/app/*                        │    │
│  └────────────────────────────────────┘    │
│                                             │
│  ┌────────────────────────────────────┐    │
│  │  Persistent Volume                 │    │
│  │  /app/model_cache                  │    │
│  │  (Model weights stored here)       │    │
│  └────────────────────────────────────┘    │
│                                             │
│  EXPOSE: 8000                               │
└────────────────────────────────────────────┘
```

## 🌐 Deployment Architecture (Render)

```
Internet
    │
    │ HTTPS
    ▼
┌─────────────────────────────────────┐
│   Render Load Balancer              │
│   • SSL/TLS termination             │
│   • Health checks                   │
└─────────────────┬───────────────────┘
                  │
                  ▼
┌─────────────────────────────────────┐
│   Docker Container                  │
│   • Auto-scaling                    │
│   • Auto-restart                    │
│   • Log aggregation                 │
└─────────────────┬───────────────────┘
                  │
                  ▼
┌─────────────────────────────────────┐
│   Persistent Disk (5GB)             │
│   • Model cache                     │
│   • Survives restarts               │
└─────────────────────────────────────┘
```

## 📦 Technology Stack

```
Frontend Layer
├─► Next.js 14 (App Router)
├─► React 18
├─► TypeScript
├─► Tailwind CSS
└─► Axios

Next.js Backend Layer
├─► Next.js API Routes
├─► MongoDB (Mongoose)
├─► JWT Authentication
├─► AWS S3 (AWS SDK v3)
└─► bcryptjs

Python Backend Layer
├─► FastAPI
├─► PyTorch
├─► Transformers (Hugging Face)
├─► Indic-TrOCR Model
├─► OpenCV
├─► Pillow
└─► Pydantic

Infrastructure
├─► Docker
├─► Render (Python backend)
├─► Vercel (Next.js - optional)
├─► MongoDB Atlas
└─► AWS S3
```

## 🔄 Model Loading & Caching

```
First Request:
    │
    ├─► Model not in cache
    │
    ├─► Download from Hugging Face
    │   ├─► Vision encoder (~500MB)
    │   ├─► Text decoder (~500MB)
    │   └─► Tokenizer configs
    │
    ├─► Save to persistent disk
    │   └─► /app/model_cache/
    │
    ├─► Load into memory
    │
    └─► Ready for inference

Subsequent Requests:
    │
    ├─► Model in cache ✓
    │
    ├─► Load from disk (fast)
    │
    └─► Ready for inference
```

## 🎯 Scalability Considerations

### Horizontal Scaling (Multiple Instances)
```
Load Balancer
    │
    ├─► Instance 1 (Model loaded)
    ├─► Instance 2 (Model loaded)
    └─► Instance 3 (Model loaded)
```

### Vertical Scaling (Bigger Instance)
```
CPU → GPU
2GB RAM → 8GB RAM
1 vCPU → 4 vCPU
```

### Async Processing (Future Enhancement)
```
User Request
    │
    ├─► Queue (Redis/Celery)
    │
    ├─► Background Worker
    │   └─► Process OCR
    │
    └─► Webhook callback
```

---

This architecture provides:
- ✅ Separation of concerns
- ✅ Scalability
- ✅ Maintainability
- ✅ Security (JWT auth)
- ✅ Performance (model caching)
- ✅ Reliability (health checks, error handling)
